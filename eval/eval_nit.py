...
def exact_match_f1(pred, answer_list):
    ...
    pred_words = set(re.findall(r"\w+", pred.lower().replace("the answer is",""))[:5]) # @$@ RIOS  # Convert the prediction into a set of words for faster operations
...
def main():
    ...
    # pred_list = load_funky_json('/home/dan/DeepLearning/mini_temporal/generations/TQE_IT/gemma-1_1-2b-it/no_context_trained/no_context_evaluated/TQE_gemma-1_1-2b-it_no_context_trained_no_context_evaluated.jsonl')
    ...
    # actual_df = pd.read_json('/home/dan/DeepLearning/mini_temporal/clean_for_fatemeh/new_data/test.jsonl', lines=True)

    # there are multiple answers, we will check them all. split into list
    actual_df['answer'] = actual_df['answer'].apply(lambda ans: ans[0].split('__or__'))

    # Split to just grab the question (we will use this for merging)
    pred_df['QUESTION'] = pred_df['QUESTION'].apply(lambda x: x.split(' Here is the context:')[0].strip())

    # rename columns to match 
    pred_df.rename(columns={'QUESTION': 'question', 'PREDICTION': 'prediction'}, inplace=True)

    # merge on questions columns
    merged_df = pd.merge(actual_df, pred_df, on='question', how='left')
    # for pred in merged_df['prediction'].head(10):
    #     print('---------------------------------------------------')
    #     print(pred)
    ...
    # CALULATE ACC BEFORE CLEANING UP PREDICTION STRING
    contains = []
