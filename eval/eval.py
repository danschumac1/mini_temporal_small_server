...
# =============================================================================
# DEBUGGIN STEPS
    # LOOK AT OUTPUTS FOR NO_CONTEXT_TRAINED FILES. ARE THEY DIFFERENT? 
    # DEBUGGING PRINTS.
    # MAKE SURE THE CHANGES DON'T MESS UP OTHER STUFF.

    #     model_generations['question'] = model_generations['question'].apply(lambda x: split_if_context(x).strip()) 
        #  AttributeError: 'NoneType' object has no attribute 'strip'

# CUDA_VISIBLE_DEVICES=1 python eval.py --pred_file "/home/dan/DeepLearning/mini_temporal/generations/output/TQE/gemma-7b/wrong_date_context_trained/no_context_evaluated/TQE_gemma-7b_wrong_date_context_trained_no_context_evaluated.jsonl" --actual_list "./eval_data1/tqe_actual.jsonl" --actual_df "../clean_for_fatemeh/new_data/test.jsonl" > minify_results.jsonl
...
def parse_args():
    ...
    parser.add_argument('--actual_list', type = str, default="./eval_data1/tqe_actual.jsonl")
    parser.add_argument('--pred_file', type=str, required=True, help='Path to the prediction JSONL file')
    parser.add_argument('--actual_df', type=str, required=True, help='Path to the actual data JSONL file')
...
def bertify_example(example, tokenizer, cls_id, sep_id):
    ...
    reference = tokenizer.tokenize(preprocess(example['reference'])).merge_dims(1, 2)
...
def split_if_context(question):
    if 'here is the context' in question:
        processed_q = question.split('here is the context')[0]
        return processed_q
    else:
        return question
    
def delete_question_word(question):
    if question.startswith('question '):
        processed_q = question[len('question '):]
        return processed_q
    else:
        return question
...
def main():
    ...
    model_generations = pd.DataFrame(clean_list_of_dicts)

    actual = pd.read_json(args.actual_list, lines=True)
    ...
    actual_df = pd.read_json(args.actual_df, lines=True, orient='records')
    
    model_generations.rename(columns={'QUESTION': 'question', 'PREDICTION': 'prediction'}, inplace=True)

    for col in actual_df.columns:
        try:
            actual_df[col] = actual_df[col].apply(lambda x: preprocess(x).strip())
        except: # won't work for interger values etc.
            pass
    
    for col in model_generations.columns:
        try:
            model_generations[col] = model_generations[col].apply(lambda x: preprocess(x).strip())
        except: # won't work for interger values etc.
            pass
    # print(model_generations['question'].iloc[0])
    # print(actual_df['question'].iloc[0])
    # print(actual_df['question'] == model_generations['question'])
    # for question in model_generations['question']:
    #     print(split_if_context(question))
    model_generations['question'] = model_generations['question'].apply(lambda x: split_if_context(x)) # .strip()
    
    model_generations['question'] = model_generations['question'].apply(lambda x: delete_question_word(x).strip())

    merged_df = pd.merge(actual_df, model_generations, on='question', how='left')
